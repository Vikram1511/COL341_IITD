...
#%%
#    alpha_betas = [[0.1,0.5],[0.2,0.8]]
   learning_rates=[0.01,0.1,0.3,1,10]
   cost_hist_hist=[]
   theta_hist=[]
   for i,lr in enumerate(learning_rates):
        # al = alpha_betas[i][0]
        # be = alpha_betas[i][1]
        cost_hist,theta,grad = run_model(trainX_encoded,trainY,lr,1000,class_weights=class_weights)
        cost_hist_hist.append(cost_hist)
...
#%%
   for i,theta in enumerate(theta_hist):
        ...
        save_image(df_per_class,"confusion_table_lr_"+str(learning_rates[i])+"_case.png")
        ...
        save_image(df_micro_macro,"scores_table_lr_"+str(learning_rates[i])+"_.png")
   fig,ax = plt.subplots()
   plt.xlabel("iterations")
   plt.ylabel("cost(log likelihood)")
   plt.title("Cost for multiple learning_rates for backtracking")

   for i,cost in enumerate(cost_hist_hist):
       ax.plot(cost,label="learning_rate:"+str(learning_rates[i]))
